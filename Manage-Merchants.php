...
<link rel="manifest" href="res/img/site.webmanifest">
  ...
  <body>
        ...
        <div class="container">
          <div class="page-inner">
            <div class="page-header">
              <h3 class="fw-bold mb-3"> Merchants</h3>
    ...
    <div class="mb-3">
        <label for="subjectSelect" class="form-label">Category</label>
    ...
    <div class="mb-3">
        <label for="paperFile" class="form-label">Upload the Merchants ( Use PNG or JPG )</label>
        <input type="file" class="form-control" id="paperFile" name="paper_file" accept=".jpg,.pdf,.png,.jpeg" required>
    </div>
</form>

<script>
document.getElementById('addPaperForm').addEventListener('submit', function(e) {
    // Set hidden fields to empty value
    document.getElementById('paperYear').value = '-';
    document.getElementById('paperTerm').value = '-';
    document.getElementById('paperMedium').value = '-';
});
</script>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="addPaperBtn">Add Item</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editPaperForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label for="editSubjectSelect" class="form-label">Category</label>
                    ...
                    <div class="mb-3">
                        <label for="editPaperFile" class="form-label">Upload the Merchants (Use PNG or JPG)</label>
                        <input type="file" class="form-control" id="editPaperFile" name="paper_file" accept=".jpg,.pdf,.png,.jpeg" required>
                    </div>
                </form>

                <script>
                    document.getElementById('editPaperForm').addEventListener('submit', function(e) {
                        // Set hidden fields to default values
                        document.getElementById('editPaperYear').value = '-';
                        document.getElementById('editPaperTerm').value = '-';
                        document.getElementById('editPaperMedium').value = '-';
                    });
                </script>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="updatePaperBtn">Update Item</button>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
...
function initializeForm(gradeId) {
    ...
    fetch(`get_subjects_by_grade.php?grade_id=${gradeId}`)
        .then(response => response.json())
        .then(data => {
            subjectSelect.innerHTML = '<option value="">Select Category</option>';
...
function fetchSubjectsForEditModal(selectedSubject) {
    ...
    fetch(`get_subjects_by_grade.php?grade_id=${encodeURIComponent(gradeId)}`)
        ...
        .then(data => {
            ...
            editSubjectSelect.innerHTML = '<option value="">Select Category</option>';
            data.forEach(subject => {
                const option = document.createElement('option');
                option.value = subject.id;
                option.text = subject.name;
